...
typedef enum Algorithm_Runner_Result {
	...
	*/
	...
	/**
		@brief The related operation failed because an user provided `Algorithm`
		failed to sort an array
	*/
	ALGORITHM_RUNNER_RESULT_SORTING_ERROR,
	/**
		@brief The related operation failed because an IO operation (such as 
		`fopen`) failed
	*/
	ALGORITHM_RUNNER_RESULT_IO_ERROR,
	/**
		@brief The related operation failed because a memory related operation
		(such as `malloc`) failed
	*/
...
/**
	...
	@brief An user provided callback that, in combination with an `Algorithm`,
	defines an algorithm. Given an array and its size this callback must sort
	it, whilst writing the appropriate values into the provided statistics
	pointer.
...
/**
	...
	@brief An user provided callback that defines how the number of element in
	the array to sort should be increased. It also determines when to stop
	running a series of tests.

	@param element_count A pointer to the current number of elements. Must be
	modified with the next element count.
...
/**
	...
	@brief Describes the configuration to use while initializing a determined 
	`Algorithm_Runner`. All the fields in this structure are user defined and
	are expected to be valid for the entire `Algorithm_Runner` lifetime.
*/
typedef struct Algorithm_Runner_Descriptor {
	/**
		@brief The list of algorithm to test and benchmark. Its length is 
		specified in `algorithms_count`.
		@note The list and its contents are expected to be valid for the entire
		lifetime of the related `Algorithm_Runner`.
		@note Must not be NULL.
	*/
	...
	*/
	...
	/**
		@brief The function used to increment the element count between tests
		@note Must not be NULL.
	*/
	Element_Count_Incrementer element_count_incrementer;

	/**
		@brief The path to the average statistics output file. This is where the
		results of the tests with the same number of elements are put after they
		are averaged.
		@note Expected to be valid for the entire lifetime of the related
		`Algorithm_Runner`.
		@note Must not be NULL.
	*/
	...
	*/
	...
	/**
		@brief The number of tests that must be executed per number of elements
		to sort
		@note Must be >= 1.
	*/
...
/**
	...
	`Algorithm_Runner_Descriptor`, it tests and benchmarks the given algorithms.
	The results of the tests are written in the following files as csv:
		- single test data: Written to the file specified in the descriptor as
		`single_test_stats_output_file`. These statistics are not processed in
		any way.
		- averaged data: Written to the file specified in the descriptor as
		`average_stats_output_file`. These results are obtaining by averaging
		the tests that have been run with the same number of elements.
	In both files the results are written in sections, one for every algorithm.
	Each section is specified by a new empty line (or the start of the file) and
	another line containing the name of the algorithm related to the section.
	Inside a section, the data is written in the following formats:
		- single test data: 
			element count, run number, exection time in ns, comparisons, swaps,
			bytes allocated
		- averaged data
			element count, exection time in ns, comparisons, swaps, bytes
			allocated
*/
...
/**
	...
	- `ALGORITHM_RUNNER_RESULT_INVALID_PARAMETER` if `runner` or `descriptor`
		are NULL
	- `ALGORITHM_RUNNER_RESULT_INVALID_DESCRIPTOR` if the requirements defined
		in the documentation of the `descriptor` are not fulfilled
	- `ALGORITHM_RUNNER_RESULT_IO_ERROR` if it could not be possible to open the
...
/**
	...
	@return One of the following:
	- `ALGORITHM_RUNNER_RESULT_SORTING_ERROR` if an algotithm fails to sort an
		array
	- `ALGORITHM_RUNNER_RESULT_MEMORY_ERROR` if the runner could not allocate 
		the memory necessary to run the tests
*/
